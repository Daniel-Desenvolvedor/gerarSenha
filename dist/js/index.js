...
const srvcEscolhido = document.getElementById('servico');
...
function opcaoOutro() {
    ...
    const indcSrvcEscolhido = srvcEscolhido.selectedIndex;
    // Variável que obterá a opção escolhida pelo usuário através do índice.
    const opEscolhida = srvcEscolhido.options[indcSrvcEscolhido];
...
function gerarSenha() {
    ...
    const qtdCaractsSenha = document.getElementById("qtdCaracts");
    // Definindo como número o valor da quantidade de caracters da senha e armazenando na variável.
    const caractsSenha = parseInt(qtdCaractsSenha.value);
    ...
    while (senha_usuario.length < caractsSenha) {
        ...
        let optGrSenh = Math.floor(Math.random() * 4);
...
function removerSevicos_Senhas() {
    // Declaração da variável que receberá a div com a id 'sevicos_Senhas' da página 'index.html'.
    const divSevicos_Senhas = document.getElementById('sevicos_Senhas');
...
function copiarSenha(senha_recebida) {
